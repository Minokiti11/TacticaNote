...
Rails.application.configure do
  ...
  config.active_storage.urls_expire_in = 2.weeks
  ...
  config.action_mailer.default_url_options = {  host: 'tactica-note.com', port: 8000 }
  ...
  config.action_cable.url = "ws://tactica-note.com/cable" 
  config.action_cable.allowed_request_origins = ['https://tactica-note.com']

  config.active_job.queue_adapter = :sidekiq
  # この設定により、ActiveJobがジョブをキューに入れる際にSidekiqを使用するようになる
  config.cache_store = :redis_cache_store, { url: ENV['REDIS_URL'] }
  config.session_store :cache_store, expire_after: 90.days #セッションの有効期限は1週間
