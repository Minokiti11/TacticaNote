        ...
        if type == "good"
            ...
            if !specific
                OpenAI::Client.new.chat(
                    parameters: {
                        model: MODEL_NAME,
                        messages: [{ role: "system", content: "今から入力する文章はサッカーノートの「上手くいかなかったこと」の欄に書かれている文章です。現象が起こった理由や具体的なシチュエーションを、追加して書くように促してください。できる限り簡潔に回答し、考えられる原因や解決策は提示しないでください。チームの戦術的な原因も書くように促すこと。output only answer with 「。」." }, { role: "user", content: prompt}],
                        temperature: TEMPERATURE,
                        stream: stream_proc(type),
                        max_tokens: 200,
                        n: RESPONSES_PER_MESSAGE
