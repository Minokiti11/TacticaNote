class GetAiResponse include ActionView::RecordIdentifier
    ...
    PROMPTS_CHANGE_SUGGESTION = {
        good: "
            ...
            ですます調ではなく、だ調で出力して。
            <置き換えるユーザーの文章>には、ユーザーの文章で変更するべき部分を抽出し、挿入してください。(「#AIのフィードバック」から抽出してこないこと。)
    ...
    def perform(note_for, channel, input, type, token, user_id, group_id, response_id)
        if input && input != ""
            ...
            suggestion_response = OpenAI::Client.new.chat(
                parameters: {
                    model: MODEL_NAME,
                    messages: [{ role: "system", content: PROMPTS_CHANGE_SUGGESTION[type.to_sym] }, { role: "user", content: input + auto_addiction + "#AIによるフィードバック:\n" + message}],
                    temperature: TEMPERATURE,
                    max_tokens: 1000,
                    n: RESPONSES_PER_MESSAGE
                }
