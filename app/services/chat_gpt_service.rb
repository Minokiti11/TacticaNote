class ChatGptService
    ...
    def chat_next(prompt)
        ...
        parameters: {
            model: "gpt-4", # Required. # 使用するGPT-3のエンジンを指定
            messages: [{ role: "system", content: "今から入力する文章はサッカーノートの「次に意識すること・次までに取り組むこと」の欄に書かれている文章です。実現させるために必要なものが書かれていたら「True」を、書かれていなかったら「False」を出力してください。output only answer." }, { role: "user", content: prompt }],
            temperature: 0.5, # 応答のランダム性を指定
            max_tokens: 200,  # 応答の長さを指定
        },
        )
